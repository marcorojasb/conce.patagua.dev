...
// Split the GTFS data into two files so the heavy bus-routes (with full
// path geometries) can be lazy-loaded as a separate chunk. Stops stay
// eager because the paradero layer + paradero detail sheets need them
// on first paint.
const stopsPath = resolve('src/data/gtfs-stops.generated.ts');
const routesDataPath = resolve('src/data/gtfs-bus-routes.generated.ts');

const stopsFile = `// AUTO-GENERATED by scripts/generate-gtfs-concepcion.ts on ${summary.generatedAt}.
// GTFS paraderos (Paradero[]) — paired with gtfs-bus-routes.generated.ts.
...
const routesFile = `// AUTO-GENERATED by scripts/generate-gtfs-concepcion.ts on ${summary.generatedAt}.
// GTFS bus routes (BusRoute[]) with full path geometries — heavy chunk
// that's lazy-loaded by src/data/routes.ts on app start (non-blocking).
// Re-generate: \`npm run sync:gtfs-concepcion\`.
...
// Compat barrel for any consumer still importing from the old location —
// re-export both. The old import path keeps working but new code should
// prefer the split files for code-splitting to actually trigger.
const barrel = `// AUTO-GENERATED by scripts/generate-gtfs-concepcion.ts on ${summary.generatedAt}.
...
writeFileSync(stopsPath, stopsFile);
writeFileSync(routesDataPath, routesFile);
writeFileSync(outPath, barrel);
console.log(
